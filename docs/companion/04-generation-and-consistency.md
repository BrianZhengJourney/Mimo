...
### ✅ 已完成标定(2026-07-20,用本 app 已有的 9 只伴灵实测)

| 对比 | 距离范围 | 样本 |
|---|---|---|
| **同角色·同阶段·不同表情**(闸门必须容忍) | 4.89 – **7.90** | n=9 |
| **不同角色**(闸门必须抓住) | **11.14** – 28.99 | n=324 |
| 同角色·跨进化阶段(参考) | 8.58 – 26.41 | n=27 |

**结论:干净分离,间隔 3.2。Vision feature print 对本任务足够敏感,
暂不需要打包 Core ML DINOv2。**

采用阈值:`pass = 9.0`,`fail = 13.0`,`spread = 3.0`,`scaleVariation = 0.12`。

**三条从实测中得到、写进代码的教训:**

1. **距离尺度是 8–29,不是 0–1。** 本文档早前草稿里凭直觉写的 `0.55 / 0.9`
   会**拒绝掉有史以来生成的每一张图**。已加测试把阈值钉在实测尺度上
   (断言 pass 必须 > 7.90 且 < 11.14),防止有人改回幻想中的数字。
2. **第一次测错了对象。** 最初比较的是**进化阶段之间**的距离,结果很难看
   (最佳准确率 95.2%,而朴素基线就有 92.3%,几乎无信息量),差点据此
   误判 Vision 不够用。但**阶段本来就该不同** —— 闸门的工作是判断
   *同一阶段内*的漂移,换成表情表重测才得到上面的干净分离。
3. **格子只能与同一阶段的基准比较。** 跨阶段距离横跨了判定边界,
   拿初生当基准去量发光阶段的格子会被判成"不是同一个角色"。

**局限**:只有一只伴灵有表情表,可容忍侧 n=9。等 P3b 产出真正的 9 帧动作表后,
应当用其实际分布重新拟合。校准工具留在 `mac/tools/consistency_calibration*.swift`。
