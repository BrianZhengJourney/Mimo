...
6. gaze：上、右上、右、右下、下、左下、左、左上八方向正确；四个 cardinal
   必须在正常尺寸下无标签也可辨，尤其 `LEFT / 270°` 不得看成正面或右向；
...
- 「跟随光标」进入显式模式：角色原地不走，整个桌面范围内按 8 个方向响应，
  只保留约 80ms 的换向平滑；「回到自动」退出。日常自动模式仍使用近距离、
  慢光标、停留后才回应的安静规则。Gaze contract v2 用绝对屏幕坐标写死
  `000° / 090° / 180° / 270°`；旧的方向语义弱资产可保留使用，但 Studio 会提供
  新一轮显式重生成，不会自动调用 provider。
- 「墙边站着 / 坐着」移动到最近真实屏幕边界，先完整播放 0–2 的站立呼吸，
  再进入 3–5 的坐姿摆腿循环；每次换帧都按当前帧的实际非透明轮廓重新收进
  工作区，不只检查 anchor，因此坐姿下沿不会掉出屏幕。

仅检查 strip 存在、尺寸正确和 frame count 正确，不足以证明动作在产品里可见。
Starter 验收必须同时覆盖菜单入口、runtime 状态、分段播放和「回到自动」退出路径。
