...
struct ReflectionBrowserUITests {
    static func main() throws {
        ...
        expect(html.contains("journey-graph") && html.contains("buildJourneyModel")
               && html.contains("journey-stone")
               && html.contains("stone-trail")
               && html.contains("topicDetail")
               && html.contains("data-visit-block")
               && html.contains("scroll-thread")
               && html.contains("type:'rest'")
               && html.contains("setJourneyGraphHover")
               && html.contains("data-graph-layout=\"topic\"")
               && html.contains("data-thread-topic")
               && html.contains("let journeyGraphLayout='topic'"),
               "the journey reads as a topic garden with visit detail, then as an ink timeline with explicit returns")
